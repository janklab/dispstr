...
reporoot = dispstrlib.globals.distroot;
origDir = pwd;
RAII.cd = onCleanup(@() cd(origDir));
cd(reporoot)

fprintf('Building dispstr...\n')

cfgFile = fullfile(reporoot, '.mlproject.json');
config = jsondecode(dispstrlib.internal.util.readtext(cfgFile));
